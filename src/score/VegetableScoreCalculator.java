					...
					for(IPlayer p : players) {
						if(p.getPlayerID() != thisPlayer.getPlayerID()) {
							...
							if((criteria.indexOf("MOST")>=0) && (playerVeg > nrVeg)) {
								nrVeg = vegetableCounter.countVegetables(p.getHand(), Vegetable.valueOf(veg));
							}
				...
				//ID3, ID4, ID5, ID6, ID7, ID8, ID9, ID10, ID11, ID12, ID13, ID14, ID15, ID16, ID17
					...
					if(criteria.indexOf("+")>=0) { //ID5, ID6, ID7, ID11, ID12, ID13
						...
						if(countSameKind > 1) {
							...
							totalScore +=  ((int)vegetableCounter.countVegetables(hand, Vegetable.valueOf(vegs[0].trim()))/countSameKind) * Integer.parseInt(criteria.split("=")[1].trim());
						} else {
							for(int i = 0; i < vegs.length; i++) {
								nrVeg[i] = vegetableCounter.countVegetables(hand, Vegetable.valueOf(vegs[i].trim()));
							}
						...
						for(int i = 0; i < parts.length; i++) {
							...
							totalScore += Integer.parseInt(veg[0].trim()) * vegetableCounter.countVegetables(hand, Vegetable.valueOf(veg[1].trim()));
						}
					}
				}
			}
